...
cat("Starting Music Variance + Random Slopes Model...\n")
...
formula_mod <- bf(
  ...
  disc ~ 1 + educ_c + social_c + economic_c + (1 + educ_c + social_c + economic_c | genre)
)
...
saveRDS(fit, file = here::here("cache", "music_hier_5_var_rs.rds"))
fit <- add_criterion(fit, "waic", ndraws = 1000)
saveRDS(fit, file = here::here("cache", "music_hier_5_var_rs.rds"))
cat("Finished Music Model 5!\n")
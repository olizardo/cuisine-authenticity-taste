...
formula_mod <- bf(
    rating_ord ~ 1 + educ_c + peduc_c + social_c + economic_c + income_c + age_c + arts_c + gend.f + race.f + (1 | respondent_id) + (1 | cuisine),
    disc ~ 1 + educ_c + peduc_c + social_c + economic_c + arts_c + (1 | cuisine)
  )

cat("Fitting Model 5: Variance Strict...\n")
fit <- brm(
  formula = formula_mod,
  data = dat_long,
  family = acat("logit"),
...
cat("Saving...\n")
saveRDS(fit, file = here::here("cache", "hier_4_var.rds"))
gc()

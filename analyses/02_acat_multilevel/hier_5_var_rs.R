...
formula_mod <- bf(
    rating_ord ~ 1 + social_c + economic_c + educ_c + age.f + race.f + gend.f + inc.f + arts.f + (1 | respondent_id) + (1 + social_c + economic_c + educ_c | cuisine),
    disc ~ 1 + social_c + economic_c + educ_c + (1 + social_c + economic_c + educ_c | cuisine)
  )

cat("Fitting Model 5: Variance + Random Slopes Strict...\n")
fit <- brm(
  formula = formula_mod,
  data = dat_long,
  family = acat("logit"),

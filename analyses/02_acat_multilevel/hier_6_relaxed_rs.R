...
formula_mod <- bf(rating_ord ~ 1 + cs(social_c) + cs(economic_c) + cs(educ_c) + age.f + race.f + gend.f + inc.f + arts.f + (1 | respondent_id) + (1 + social_c + economic_c + educ_c | cuisine))

cat("Fitting Model 6: Relaxed CS + Random Slopes...\n")
fit <- brm(
  formula = formula_mod,
  data = dat_long,
  family = acat("logit"),

...
formula_mod <- bf(rating_ord ~ 1 + cs(social_c) + cs(economic_c) + cs(educ_c) + age.f + race.f + gend.f + inc.f + arts.f + (1 | respondent_id) + (1 | cuisine))

cat("Fitting Model 2: Relaxed CS...\n")
fit <- brm(
  formula = formula_mod,
  data = dat_long,
  family = acat("logit"),
...
cat("Saving...\n")
saveRDS(fit, file = here::here("cache", "hier_2_relaxed.rds"))
gc()
